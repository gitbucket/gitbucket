...
import gitbucket.core.util.{ReadableUsersAuthenticator, ReferrerAuthenticator, RepositoryName, UnarchivedAuthenticator}
...
trait ApiIssueCommentControllerBase extends ControllerBase {
   ...
   */
  patch("/api/v3/repos/:owner/:repository/issues/comments/:id")(unarchivedRepositoryOnly {
    readableUsersOnly {
      repository =>
   ...
   */
  delete("/api/v3/repos/:owner/:repo/issues/comments/:id")(unarchivedRepositoryOnly {
    readableUsersOnly {
      repository =>
   ...
   */
  post("/api/v3/repos/:owner/:repository/issues/:id/comments")(unarchivedRepositoryOnly {
    readableUsersOnly {
      repository =>
        (for {
          issueId <- params("id").toIntOpt
          ...
          action = params
            .get("action")
            .filter(_ => isEditable(issue.userName, issue.repositoryName, issue.openedUserName))
