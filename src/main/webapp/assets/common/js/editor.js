(function () {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    markdownBold.addEventListener('click', (e) => {
      const selectedText = gitbucket.editor.getSelectedText();
      if (selectedText.length > 0) {
        gitbucket.editor.insert("**" + selectedText + "**");
      }
      markdownBold.blur();
    ...
    markdownCode.addEventListener('click', (e) => {
      let selectedText = gitbucket.editor.getSelectedText();
      if (selectedText.length > 0) {
        if (selectedText.includes('\n')) {
          const range = gitbucket.editor.getSelectionRange();
          ...
          gitbucket.editor.selection.selectLineEnd();
          selectedText = gitbucket.editor.getSelectedText();
          gitbucket.editor.insert("```\n" + selectedText + (selectedText.endsWith() ? "```" : "\n```"));
    ...
    markdownLink.addEventListener('click', (e) => {
      gitbucket.editor.insert("[" + gitbucket.editor.getSelectedText() + "]()");
      markdownLink.blur();
      gitbucket.editor.focus();
    });
        ...
        switch(lineType[0]) {
          case TYPE_HEADING:
            ...
            if (level < 6) {
              ...
              gitbucket.editor.insert(selectedText.replace("###### ",""));
            }
            break;
          case TYPE_NONE:
            gitbucket.editor.insert("# " + selectedText);
            break;
          default:
            gitbucket.editor.insert(selectedText.replace(lineType[1][0], "# "));
        }
      }
        ...
        switch(lineType[0]) {
          case TYPE_QUOTE:
            if (m = selectedText.match(nestedQuotePattern)) {
              gitbucket.editor.insert(selectedText.replace(m[0], ""));
            } else {
              gitbucket.editor.insert("> " + selectedText);
            }
            break;
      ...
      for (let row = range.start.row; row <= range.end.row; row++) {
        ...
        switch(lineType[0]) {
          case TYPE_OL:
            ...
            break;
          case TYPE_NONE:
            gitbucket.editor.insert("1. " + selectedText);
            break;
          default:
            gitbucket.editor.insert(selectedText.replace(lineType[1][0], "1. "));
        }
      }
      markdownListOl.blur();
      gitbucket.editor.focus();
    ...
    const updateFileMode = function () {
      if (gitbucket.getFileName() != "") {
        ...
        const mode = modelist.getModeForPath(gitbucket.getFileName());
    ...
    const aceKeyboard = localStorage.getItem("gitbucket:editor:keyboard") || "";
    gitbucket.editor.setKeyboardHandler(aceKeyboard == "" ? null : aceKeyboard);

    const aceKeyboardSelect = document.getElementById("aceKeyboardSelect");
      ...
      $.get(gitbucket.isRenderableUrl + gitbucket.getFileName(), function (data) {
        if (data === 'true') {
          ...
          $.post(gitbucket.previewUrl, {
            ...
            $('#preview').empty().append($('<div class="markdown-body" style="padding-left: 20px; padding-right: 20px;">').html(data));
            prettyPrint();
          });
        } else {
          // Show diff
      ...
      try {
        ...
        $('#editor').dropzone({
          ...
          success: function (file, id) {
            const attachFile = (file.type.match(/image\/.*/) ? '\n![' + file.name.split('.')[0] : '\n[' + file.name) + '](' + file.name + ')';
